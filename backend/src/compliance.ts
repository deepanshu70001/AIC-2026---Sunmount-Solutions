type FilingStatus = 'COMPLIANT' | 'RETURN_PENDING' | 'BLOCKED';
type ItcRisk = 'LOW' | 'MEDIUM' | 'HIGH';

type PartyComplianceProfile = {
  partyId: string;
  gstin: string;
...
export type ComplianceOrderLine = {
  product_code: string;
  quantity: number;
  price: number;
  hsn_code?: string;
...
export type DispatchComplianceResult = {
  invoice_number: string;
  transport_details: string;
  eway_bill_required: boolean;
  eway_bill_status: string;
  eway_bill_number?: string;
  eway_bill_generated_at?: Date;
  eway_bill_valid_upto?: Date;
...
const BLOCKED_PARTIES = new Set(
  String(process.env.COMPLIANCE_BLOCKED_PARTIES || '')
    .split(',')
    .map(value => value.trim().toUpperCase())
    .filter(Boolean)
);

const COMPLIANCE_PROFILES: Record<string, PartyComplianceProfile> = {
  'CUST-1001': {
    partyId: 'CUST-1001',
...
const deriveHsnCode = (productCode: string) => {
  const normalized = String(productCode || '').toUpperCase();
  if (normalized.startsWith('STL') || normalized.startsWith('SS-')) return '7214';
  if (normalized.startsWith('COP')) return '7408';
  if (normalized.startsWith('PVC')) return '3917';
  if (normalized.startsWith('IND-BLT')) return '7318';
  if (normalized.startsWith('BRG')) return '8482';
  if (normalized.startsWith('WLD')) return '8311';
  if (normalized.startsWith('ALM')) return '7606';
  return '8479';
};

export const extractPartyId = (value: string | null | undefined) => {
  if (!value) return null;
  const match = String(value).toUpperCase().match(/([A-Z]{3,5}-\d{3,6})/);
  return match?.[1] || null;
};

const resolvePartyCompliance = (partyId: string | null): PartyComplianceProfile => {
...
const normalizeTransportDetails = (raw: any): TransportDetails => {
  ...
  const mode: TransportDetails['mode'] = ['ROAD', 'RAIL', 'AIR', 'SHIP'].includes(modeRaw)
    ? (modeRaw as TransportDetails['mode'])
    : 'ROAD';

  return { vehicle_number, transporter_id, distance_km, mode };
};

const calculateTaxSummary = (orderLines: ComplianceOrderLine[], products: ComplianceProduct[]): TaxSummary => {
...
const requestEwayBill = async (payload: any, orderId: string, distanceKm: number) => {
...
export const evaluateDispatchCompliance = async (input: {
  orderId: string;
  customerSupplierId: string | null | undefined;
  invoiceNumber?: string | null;
  products: ComplianceOrderLine[];
  productCatalog: ComplianceProduct[];
  transportDetails?: any;
}): Promise<DispatchComplianceResult> => {
  const partyId = extractPartyId(input.customerSupplierId);
  ...
  if (complianceProfile.filingStatus !== 'COMPLIANT') {
    throw new Error(
      `E-Way Bill blocked for ${complianceProfile.partyId}. ${complianceProfile.reason}`
    );
  }
...
export const buildComplianceSummary = async (prisma: any) => {
  ...
  const salesDispatch = orders.filter((order: any) => order.type === 'SALE' && order.status === 'DISPATCH');
  const salesOpen = orders.filter((order: any) => order.type === 'SALE' && order.status !== 'DISPATCH');
  const ewayGenerated = salesDispatch.filter((order: any) => order.eway_bill_status === 'GENERATED').length;
  const ewayRequiredPending = salesDispatch.filter((order: any) => order.eway_bill_required && !order.eway_bill_number).length;
  ...
  for (const order of salesOpen) {
    ...
    })() as ComplianceOrderLine[];
    const taxSummary = calculateTaxSummary(lines, products);
    const profile = resolvePartyCompliance(extractPartyId(order.customer_supplier_id));
  ...
  const purchaseOrders = orders.filter(
    (order: any) => order.type === 'PURCHASE' && ['PAID', 'COMPLETED'].includes(String(order.status || '').toUpperCase())
  );
