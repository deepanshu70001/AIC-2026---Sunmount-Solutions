 ...
 */
function generateForecasts(orders, products, manufacturing) {
    const now = new Date();
    ...
    const salesModel = regression_1.default.linear(salesData);
    // Predict next 7 days
    const futureSales = [];
    for (let i = 1; i <= 7; i++) {
    ...
    const stockoutWarnings = products.map(p => {
        ...
        salesOrders.forEach(o => {
            try {
                const items = typeof o.products === 'string' ? JSON.parse(o.products) : o.products;
                if (Array.isArray(items)) {
                    const match = items.find((i) => i.product_code === p.product_code);
                    if (match)
                        sold += Number(match.quantity);
                }
            }
            catch (e) {
    ...
    const mfgData = []; // [total_output_qty, duration_hours]
    completedBatches.forEach(b => {
        ...
        try {
            ...
            if (Array.isArray(output)) {
                const qty = output.reduce((a, o) => a + Number(o.quantity), 0);
    ...
    if (mfgData.length > 2) {
        const mfgModel = regression_1.default.polynomial(mfgData, { order: 2 });
