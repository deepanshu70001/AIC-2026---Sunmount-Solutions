...
exports.generateInventoryPdf = generateInventoryPdf;
const pdfkit_1 = __importDefault(require("pdfkit"));
/**
 * Generates an official PDF invoice/bill for sales and purchase orders.
 */
function generateOrderPdf(order, dbProducts) {
    ...
    if (order.eway_bill_number || order.compliance_meta) {
        ...
        if (order.eway_bill_number)
            doc.fillColor('#44474c').font('Helvetica').text(`E-Way Bill: ${order.eway_bill_number}`, 50, y + 15);
    }
    // Footer
 ...
 */
function generateInventoryPdf(products) {
    const doc = new pdfkit_1.default({ margin: 50, size: 'A4' });
    doc.fillColor('#2d3a4a').fontSize(24).font('Helvetica-Bold').text('INVENTORY PRO', 50, 50);
    doc.fontSize(10).font('Helvetica').fillColor('#74777d')
